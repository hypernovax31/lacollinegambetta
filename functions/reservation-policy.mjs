...
export function slots() {
  const result = [];
  for (let minutes = POLICY.firstSlotMinutes;
       minutes <= POLICY.lastSlotMinutes;
       minutes += POLICY.slotStepMinutes) {
    result.push({
...
/** Chaque réservation occupe [début, début + 60 min). Les fins à la même
 * minute sont traitées avant les débuts : 12h00 et 13h00 ne se chevauchent
 * pas, tandis que 12h15 et 13h00 se chevauchent. */
export function evaluateCapacity(rows, candidate) {
  const reservations = rows
    .filter((row) => POLICY.activeStatuses.includes(String(row.status).toUpperCase()))
